...
func (r *Repository) GetRequestPaint(paintId int, requestId int) (ds.RequestsPaint, error) {
	var requestPaint ds.RequestsPaint
	err := r.db.Where("paint_id = ? and request_id = ?", paintId, requestId).First(&requestPaint).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ds.RequestsPaint{}, fmt.Errorf("%w: request paint not found", ErrNotFound)
		}
...
func (r *Repository) GetRequestPaintsList(id int) ([]ds.Paint, ds.PaintRequest, error) {
	...
	var paints []ds.Paint
	sub := r.db.Table("request_paints").Where("request_id = ?", request.ID)
...
func (r *Repository) CheckCurrentRequestDraft(creatorID uint) (ds.PaintRequest, error) {
...
func (r *Repository) GetRequestDraft(creatorID uint) (ds.PaintRequest, bool, error) {
...
func (r *Repository) GetPaintCount(creatorID uint) int64 {
	if creatorID == 0 {
		return 0
	}
...
func (r *Repository) DeleteRequest(requestId int) error{
	return r.db.Exec("UPDATE paint_requests SET status = 'удалён' WHERE id = ?", requestId).Error
}
...
func (r *Repository) ModerateRequest(id int, status string) (ds.PaintRequest, error) {
